...
function importRoleLibrary(db, libraryPath) {
  ...
  if (!fs.existsSync(resolved)) {
    return { templatesCreated: 0, filesImported: 0, error: 'Path does not exist' };
  }
  if (!fs.statSync(resolved).isDirectory()) {
    return { templatesCreated: 0, filesImported: 0, error: 'Path is not a directory' };
  }
  ...
  return db.transaction(() => {
    ...
    // Import global files
    const globalDir = path.join(resolved, 'global');
    ...
    // Import role files
    const rolesDir = path.join(resolved, 'roles');
